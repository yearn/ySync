...
function	VaultEntity({
			...
			fix: {
				...
				instructions: [
					<span key={'step-1'}>
						...
						<a
							href={'https://github.com/LedgerHQ/app-plugin-yearn/blob/develop/tests/yearn/b2c.json'}
							target={'_blank'}
							className={'underline'}
							rel={'noreferrer'}>
					...
					<span key={'step-2'}>
						...
						<code
							...
							className={'rounded-default cursor-copy bg-neutral-200 py-1 px-2 text-sm'}>
							{'contracts'}
						</code>
						{' object in the '}
						<code
					...
					<span key={'step-3'}>
						...
						<a
							href={'https://github.com/LedgerHQ/app-plugin-yearn/tree/develop/tests/yearn/abis'}
							target={'_blank'}
							className={'underline'}
							rel={'noreferrer'}>
					...
					<span key={'step-5'}>
						...
						<a
							href={'https://github.com/LedgerHQ/app-plugin-yearn/blob/develop/src/main.c#L51'}
							target={'_blank'}
							className={'underline'}
							rel={'noreferrer'}>
			...
			fix: {
				...
				instructions: [
					<span key={'step-1'}>
						...
						<a
							href={`https://github.com/yearn/ydaemon/tree/main/data/meta/strategies/${chainID}`}
							target={'_blank'}
							className={'underline'}
							rel={'noreferrer'}>
			...
			fix: {
				...
				instructions: [
					<span key={'step-1'}>
						...
						<a
							href={`https://github.com/yearn/ydaemon/tree/main/data/meta/vaults/${chainID}`}
							target={'_blank'}
							className={'underline'}
							rel={'noreferrer'}>
	...
	return (
		<div className={'rounded-default bg-neutral-100'}>
			<div className={'rounded-t-default flex flex-row space-x-4 border-b border-neutral-300 p-4'}>
				...
				<div className={'-mt-1 flex flex-col'}>
					<div className={'flex flex-row items-center space-x-2'}>
						<h4 className={'text-lg font-bold text-neutral-900'}>{vault.name}</h4>
			...
			<div className={'flex flex-col p-4 pt-0'}>
					...
					anomalies={[
						{
							...
							suffix: (
								<span className={'inline'}>
									{'for vault '}
									<a
										href={`${networks[chainID].explorerBaseURI}/address/${vault.address}`}
										target={'_blank'}
										className={`underline ${vaultData?.hasValidIcon ? 'tabular-nums' : 'tabular-nums text-red-900'}`}
										rel={'noreferrer'}>
							...
							suffix: (
								<span className={'inline'}>
									...
									<a
										href={`${networks[chainID].explorerBaseURI}/address/${vault.token.address}`}
										target={'_blank'}
										className={`underline ${vaultData?.hasValidTokenIcon ? 'tabular-nums' : 'tabular-nums text-red-900'}`}
										rel={'noreferrer'}>
							...
							suffix: (
								<span>
									...
									<a
										href={`${networks[chainID].explorerBaseURI}/address/${vault.address}`}
										target={'_blank'}
										className={`underline ${vaultData?.hasValidPrice ? '' : 'text-red-900'}`}
										rel={'noreferrer'}>
										{vaultData?.name}
									</a>
								</span>
				...
				{vaultData?.hasValidStrategiesRisk && vaultSettings.shouldShowOnlyAnomalies ? null : (
					<section aria-label={'strategies check'} className={'mt-4 flex flex-col pl-0 md:pl-0'}>
						<b className={'mb-1 font-mono text-sm text-neutral-900'}>{'Risk Score'}</b>
								...
								<StatusLine
									...
									suffix={(
										<span>
											{'for strategy '}
											<a
												href={`${networks[chainID].explorerBaseURI}/address/${strategy.address}`}
												target={'_blank'}
												className={`underline ${hasRiskFramework ? '' : 'text-red-900'}`}
												rel={'noreferrer'}>
												{strategy.name}
											</a>
										</span>
									...
									suffix={(
										<span>
											...
											<a
												href={`${networks[chainID].explorerBaseURI}/address/${strategy.address}`}
												target={'_blank'}
												className={`underline ${sum > 0 && sum < 40 ? '' : 'text-red-900'}`}
												rel={'noreferrer'}>
												{strategy.name}
											</a>
											{` (${sum})`}
				...
				{vaultData?.hasValidStrategiesDescriptions && vaultSettings.shouldShowOnlyAnomalies ? null : (
					<section aria-label={'strategies check'} className={'mt-4 flex flex-col pl-0 md:pl-0'}>
						<b className={'mb-1 font-mono text-sm text-neutral-900'}>{'Descriptions'}</b>
						{vault.strategies.map((strategy: any): ReactNode => {
									...
									suffix={(
										<span>
											...
											<a
												href={`${networks[chainID].explorerBaseURI}/address/${strategy.address}`}
												target={'_blank'}
												className={`underline ${!isMissingDescription ? '' : 'text-red-900'}`}
												rel={'noreferrer'}>
												{strategy.name}
											</a>
										</span>
				...
				{Object.keys((aggregatedData?.vaults[toAddress(vault.address)]?.missingTranslations) || []).length !== 0 && vaultSettings.shouldShowMissingTranslations ? (
					<section aria-label={'strategies check'} className={'mt-4 flex flex-col pl-0 md:pl-0'}>
						<b className={'mb-1 font-mono text-sm text-neutral-900'}>{'Missing Translations'}</b>
								...
								<StatusLine
									...
									suffix={(
										<span>
											{'for '}
											<a
												href={`${networks[chainID].explorerBaseURI}/address/${strategyAddress}`}
					...
					anomalies={[
						{
							...
							suffix: (
								<span className={'inline'}>
									{'for want token '}
									<a
										href={`${networks[chainID].explorerBaseURI}/address/${vaultData?.token?.address}`}
										target={'_blank'}
