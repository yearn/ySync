...
export function Dropdown({options, defaultOption, selected, onSelect}: TDropdownProps): ReactElement {
	return (
		<div>
			<Menu as={'menu'} className={'relative inline-block text-left'}>
				{({open}): ReactElement => {
					console.log({open: `transition-transform${open ? '-rotate-90' : '-rotate-180'}`});
					return (
						<>
							<Menu.Button
								data-variant={'light'}
